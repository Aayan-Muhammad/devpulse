...
export default function ComparePage({ params }: { params: { user1: string; user2: string } }) {
  const [data, setData] = useState<ComparisonData | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const user1Decoded = decodeURIComponent(params.user1);
  const user2Decoded = decodeURIComponent(params.user2);
  ...
  const maxFollowers = Math.max(data.user1.followers, data.user2.followers);
  const maxRepos = Math.max(data.user1.public_repos, data.user2.public_repos);
  const maxStars = Math.max(data.user1.total_stars, data.user2.total_stars);
